...
public class WordMapManager {
    ChatWatchdogBungee core;
    List<String> tempList = new ArrayList<>();
    File file;
    Lang lang = new Lang();
    public WordMapManager(ChatWatchdogBungee core) {
        this.core = core;
    ...
    public void load() {
        ...
        UtilsBungee.log(ChatColor.GREEN+lang.translate("fetched", core.wordsMap.size()+"", String.valueOf(total)));
    }
    ...
    public Boolean containsBlockedWords(ChatEvent event) {
        for (String s : core.wordsMap) {
            if (event.getMessage().contains(s)) {
                ((ProxiedPlayer) event.getSender()).sendMessage(core.getPrefix()+ChatColor.RED+lang.translate("messageBlocked", s));
                return true;
            }
        }
        return false;
